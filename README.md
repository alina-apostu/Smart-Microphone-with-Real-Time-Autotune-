...
## Completari mod de pornire (pt developer, ajuta la debug)
- prima oara porniti serverul python cu urmatoarea comanda "python server_web.py", apoi verificati in hello_pwm serial monitor sa afiseze instructiunile corespunzatoare mentionate in interfata web pentru utilizator( +parola pt Pico_Setup este 12345678, dar in interfata web nu este specificata explicit, intrucat daca ar fi un produs real, nu ar fi safe si parola default ar fi diferita pt fiecare dispozitiv), daca nu se poate conecta placuta, apasati pe butonul BOOTSEL de pe placuta, introducand placuta in PC apoi apasati CRTL+SHIFT+B pt incarcarea din nou a codului din hello_pwm pe placuta si urmariti in serial monitor pasii, pt a va asigura ca s a realizat conexiunea intre pagina web si placuta, dati inspect si in console ar trebui sa vedeti o adresa IP atribuita de wifi ul dvs pentru placuta)
- la instructiunile pt utilizator nu este precizata pornirea serverului web, intrucat daca ar fi o aplicatie si un produs real, serverul web ar fi deja disponibil utilizatorului de oriunde, dar intrucat pt acest proiect nu a fost cumparat un domeniu real, pt demo trebuie facut si acest pas in plus :)
- de asemenea, acest proiect a fost realizat pe un repo privat in colaborare cu facultatea care nu poate fi facut public, de aceea este doar un commit pt partea de cod, in realitate a fost realizat in etape
